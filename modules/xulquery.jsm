let EXPORTED_SYMBOLS = ['XULQuery', '$$'];

let XULQuery = $$ = function(window) {
	let XULQuery = function(selector) {
		return new XULQuery.fn.init(selector);
	};

    let alert = window.alert;

	// Queue onload events and run them in sequence
	XULQuery._onload = [];
	window.addEventListener('load', function(e) {
		XULQuery._onload.forEach(function(f) {
			f.call(window, e);
		});
	}, false);

	XULQuery.fn = XULQuery.prototype = {
		// Constructs a XULQuery object
		init: function(selector) {
			// Functions go to the onload queue
			if (typeof selector === "function") {
				XULQuery._onload.push(selector);
				return XULQuery;
			}

            // Handle DOMNodes
			if (selector.nodeType) {
				this[0] = selector;
				this.length++;
			}

            // Handle an array of DOMNodes
            if (Array.isArray(selector)) {
                XULQuery.merge(this, selector);
            }

			// Handle CSS selectors
			if (typeof selector === 'string') {
				XULQuery.merge(this, window.document.querySelectorAll(selector));
			}

            return this;
		},
    };

    // Extend an object with values from another object
    // If a single argument is given, extend xulquery itself
    XULQuery.fn.extend = XULQuery.extend = function() {
        let options, name,
        target = arguments[0] || {},
        k = 1;

        // Target is xulquery
        if (arguments.length === 1) {
            target = this;
            k--;
        }

        // TODO: Handle deep copy
        for (; k < arguments.length; k++) {
            if ((options = arguments[k]) != null) {
                for (name in options) {
                    if (options.hasOwnProperty(name)) {
                        target[name] = options[name];
                    }
                }
            }
        }

        return target;
    };

    XULQuery.fn.extend({
        // Find child elements matching selector
        find: function(selector) {
            // Match elements, then see if the children contain them
            let self = this, matches = XULQuery(selector);

            return matches.filter(function(val) {
                alert("filtering!");
                for (let k = 0; k < self.length; k++) {
                    if (XULQuery.contains(self[k], val)) {
                        return true;
                    }
                }

                return false;
            });
        },

        toArray: function() {
            return Array.prototype.slice.call(this, 0);
        },

        filter: function(func, thisObj) {
            let elems = this.toArray().filter(func, thisObj);
            window.alert(elems[0]);
            return XULQuery(elems);
        },

		// Binds an event
		bind: function(event, fn) {
			this.each(function(elem) {
				elem.addEventListener(event, fn, false);
			});
		},

		// Gets the value of the first matched element, or sets
		// it to the given value
		val: function(value) {
			let elem = this[0];
			if (elem) {
				if (value !== undefined) {
					elem.value = value;
				} else {
					return elem.value;
				}
			}

			return undefined;
		},

        // Retrieves the text content of all matched nodes, or sets the text
        // to the specified value.
        text: function(value) {
            if (value) {
                this.each(function(e) {
                    e.textContent = value;
                });
            } else {
                let ret = '';
                this.each(function(e) {
                    ret += e.textContent;
                });

                return ret;
            }
        },

		// Number of matched elements
		length: 0,
		each: function(func, thisObj) {
			for (let k = 0; k < this.length; k++) {
				if (this[k] !== undefined) {
					func.call(thisObj, this[k], k, this);
				}
			}
		},
	});

    XULQuery.extend({
        // Merges two arrays together
		merge: function(first, second) {
			let fst = first.length, snd = 0;
			for (let sndLen = second.length; snd < sndLen; snd++) {
				first[fst++] = second[snd];
			}
			first.length = fst;

			return first;
		},

        // Returns true if parent contains child
        contains: function(parent, child) {
            return !!(parent.compareDocumentPosition(child) & 16);
        },
    });

	// Give the init function the XULQuery prototype for instances
	XULQuery.fn.init.prototype = XULQuery.fn;

	return XULQuery;
};
